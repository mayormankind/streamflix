...
export function MovieCard({ movie }: MovieCardProps) {
  return (
    <div className="group relative aspect-[2/3] w-full min-w-[160px] cursor-pointer overflow-hidden rounded-md bg-muted transition-all duration-300 hover:scale-105 hover:z-10 md:min-w-[200px] lg:min-w-[240px]">
      {movie.media && 
        // <Image src={movie.media.thumbnail || "/placeholder.svg"} alt={movie.title} fill className="object-cover" />
        <img src={movie.media.thumbnail || "/placeholder.svg"} alt={movie.title} className="object-cover" />
      }
      <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-black via-transparent to-transparent p-4 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
        ...
        <div className="mt-2 flex items-center gap-2 text-[10px] text-muted-foreground">
          ...
          <span className="rounded border px-1 py-0.5 text-[8px]">{movie.rating}</span>
        </div>
      </div>
    </div>
  )
